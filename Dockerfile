...
# ==========================================
# 4. BACKEND CODE (app.py) - ORIGINAL RESTORED WITH YOUTUBE QUALITY FIX
# ==========================================
RUN cat << 'EOF' > app.py
...
# --- YOUTUBE DOWNLOADER WITH QUALITY SELECTOR ---
def download_youtube_video(url, quality='best', download_dir=TEMP_DIR, service=None):
    """Download YouTube video using yt-dlp with cookies and quality selector"""
    try:
        ...
        if os.path.exists(YOUTUBE_COOKIES_FILE) and os.path.getsize(YOUTUBE_COOKIES_FILE) > 100:
            ydl_info_opts['cookiefile'] = YOUTUBE_COOKIES_FILE
        ...
        with yt_dlp.YoutubeDL(ydl_info_opts) as ydl:
            ...
            logger.info(f"Available formats: {len(available_formats)}")
            for fmt in available_formats[:5]:  # Log first 5 formats
                logger.info(f"  Format: {fmt.get('format_id')} - {fmt.get('height')}p - {fmt.get('ext')} - vcodec: {fmt.get('vcodec')} - acodec: {fmt.get('acodec')}")
        
        # Map quality strings to yt-dlp format based on available formats
        format_str = get_format_for_quality(quality, available_formats)
        logger.info(f"Selected format for quality '{quality}': {format_str}")
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if quality == 'audio' and not downloaded_file.endswith('.mp3'):
                # For audio, rename to mp3
            ...
            if not os.path.exists(downloaded_file):
                ...
                for ext in ['.mp4', '.mkv', '.webm', '.mp3', '.m4a', '.flv', '.avi']:
                    ...
                    if os.path.exists(test_path):
                        ...
                        logger.info(f"Found alternate file: {test_path}")
                        break
            
            if not os.path.exists(downloaded_file):
                raise Exception(f"Downloaded file not found: {downloaded_file}")
            
            logger.info(f"Download successful: {downloaded_file} ({os.path.getsize(downloaded_file)} bytes)")
            return {
                'success': True,
...
def get_format_for_quality(quality, available_formats):
    """Get the appropriate format string based on quality selection"""
    # Default format
    if quality == 'best':
    ...
    try:
        ...
        # Find the best format with the target height
        video_formats = []
        audio_formats = []
        ...
        for fmt in available_formats:
            ...
            if vcodec != 'none' and height == target_height:
        ...
        if video_formats:
            ...
            # If no combined format, get best video and best audio separately
            best_video = max(video_formats, key=lambda x: x.get('filesize', 0))
            if audio_formats:
    ...
    try:
        ...
        if creds:
            try:
                ...
                # Load cookies to ensure we can access the video
                load_cookies_from_drive(service)
            except Exception as e:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for fmt in formats:
                ...
                if vcodec != 'none' and height > 0:
                    quality_map[height] = quality_map.get(height, {
                        'height': height,
                        'has_audio': acodec != 'none',
                        'formats': []
                    })
                    quality_map[height]['formats'].append(fmt)
    ...
    def worker():
        ...
        try:
            ...
            if action == "youtube":
                ...
                tr.update(f"Downloading from YouTube ({quality})...", 30)
...
def index():
    return jsonify({
        'status': 'TechZoneX Backend API',
        'version': '3.0',
        'youtube_support': True,
        'features': ['YouTube Downloader', 'Video Processing', 'Google Drive Integration']
    })
...
if __name__ == '__main__':
    ...
    logger.info("TechZoneX Backend starting...")
